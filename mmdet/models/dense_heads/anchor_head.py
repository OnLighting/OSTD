...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _get_targets_single(self,
        ...
        assign_kwargs = {}
        if getattr(self.assigner, 'requires_pred_scores', False):
            if pred_scores is None:
                raise ValueError(
                    f'{type(self.assigner).__name__} requires pred_scores')
            assign_kwargs['pred_scores'] = pred_scores[inside_flags].detach()
    ...
    def get_targets(self,
        ...
        if pred_scores_list is None:
            pred_scores_list = [None for _ in range(num_imgs)]
        results = multi_apply(
    ...
    def loss(self,
        ...
        cls_reg_targets = self.get_targets(
            ...
            pred_scores_list=[
                torch.cat([
                    score[img_id].permute(1, 2, 0).reshape(
                        -1, self.cls_out_channels)
                    for score in cls_scores
                ]).detach() for img_id in range(len(img_metas))
            ],
            label_channels=label_channels)
