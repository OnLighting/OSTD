...
class DetailAggregateLoss(nn.Module):
    ...
    def forward_with_gt(self, boundary_logits, fused_gt):
        """支持外部预生成的真值（用于 ARFC 改造方案 3.1）。

        Args:
            boundary_logits (Tensor): 形状 [B, 1, H1, W1]。
            fused_gt (Tensor): 已经融合过的真值，形状 [B, 1, H2, W2]。
        Returns:
            Tensor: 标量损失。
        """
        if boundary_logits.shape[-2:] != fused_gt.shape[-2:]:
            boundary_logits = F.interpolate(
                boundary_logits,
                size=fused_gt.shape[-2:],
                mode='bilinear',
                align_corners=True)
        return self.b_loss(boundary_logits, fused_gt)
