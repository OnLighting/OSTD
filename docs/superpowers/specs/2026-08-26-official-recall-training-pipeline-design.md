...
将当前以 COCO `bbox_mAP` 保存 best、按 6:2:2 划分并在独立 test 集评测的流程，改造成与比赛官方口径一致的完整训练链路：官方数据集按 8:2 划分为训练集和验证集；第一阶段在官方训练集训练；第二阶段以第一阶段 best checkpoint 为起点，使用官方训练集与 ShipRS 补充数据混合微调；两个阶段都按官方 Recall/FDR 在官方验证集上选择 best；训练期阈值按三大类自适应搜索，训练完成后再在验证集上搜索并冻结 25 类最终阈值，供模拟大图和官方独立测试集使用。
...
每轮训练验证时，分别为舰船、飞机、车辆搜索一个大类阈值。同一大类内所有子类使用同一个阈值。候选阈值由该大类全部预测的不同 score 以及“高于最大 score”的空预测点构成，不依赖手写网格。每个大类在验证集上选择满足该大类平均 `FDR <= 0.19` 的最高 Recall 操作点；Recall 相同时依次选择更低 FDR、更高阈值，确保结果确定。三个大类均满足 0.19 约束，因此其平均 official FDR 也不超过 0.19。若某个大类没有 GT，则该轮 official 指标不可用，不得保存为 best。
...
新增官方口径 best 保存 hook，读取每轮验证经过三大类阈值搜索后写入日志缓冲区的 `official_recall` 和 `official_fdr`。保存文件固定为 `best_official_recall_fdr.pth`，并在同目录 `best_official_recall_fdr.json` 中记录 epoch、Recall、FDR、三个训练期阈值和是否双达标，同时写入训练日志。
...
- 保存 25 类名称、大类映射和分类 IoU，并提供统一候选下限、三大类阈值搜索及阈值文件校验接口。
...
- 训练期任一大类找不到有效操作点或缺少 GT 时，该轮 official 指标不可用；不得静默回退到统一阈值或历史 CSV。
...
- 为三大类阈值搜索补充 FDR 0.19 边界、Recall/FDR/阈值确定性 tie-break、缺少大类 GT 和跨大类阈值独立性测试。
